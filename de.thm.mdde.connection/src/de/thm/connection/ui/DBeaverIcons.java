...
public class DBeaverIcons {
	...
	private static IconDescriptor getIconDescriptor(DBPImage image) {
		if (image == null) {
			...
			return new IconDescriptor("[" + image.getLocation() + "]", ((DBIconBinary) image).getImage(),
	...
	private static IconDescriptor getCompositeIcon(IconDescriptor mainIcon, DBIconComposite image) {
		...
		String compositeId = mainIcon.id + "^" + (image.getTopLeft() == null ? "" : image.getTopLeft().getLocation())
...
//            }
	...
	@NotNull
	private static ImageDescriptor[] accumulateDecorations(@NotNull DBIconComposite image,
