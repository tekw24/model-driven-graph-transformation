 ...
 */
package de.thm.mdde.driver.runtime.model;
...
public class DefaultProgressMonitor implements DBRProgressMonitor {
    ...
    {
        if (states.length == 0) {
            log.trace(new DBException("Progress ended without start"));
        } else {
    ...
    {
        if (states.length == 0) {
            log.trace(new DBException("Progress sub task without start"));
        } else {
    ...
    {
        if (states.length == 0) {
            log.trace(new DBException("Progress info without start"));
        } else {
