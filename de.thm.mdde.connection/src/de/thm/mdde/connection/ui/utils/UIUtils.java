...
import de.thm.connection.ui.RunnableWithResult;
import de.thm.connection.ui.UIIcon;
import de.thm.mdde.connection.internal.UIActivator;
import de.thm.mdde.connection.model.Log;
import de.thm.mdde.connection.utils.ArrayUtils;
...
import de.thm.mdde.driver.runtime.DBWorkbench;
...
public class UIUtils {
	...
	public static VerifyListener getNumberVerifyListener(Locale locale) {
		...
		return e -> {
			for (int i = 0; i < e.text.length(); i++) {
				char ch = e.text.charAt(i);
				if (!Character.isDigit(ch) && !ArrayUtils.contains(allowedChars, ch)
	...
	public static void createToolBarSeparator(Composite toolBar, int style) {
		...
		label.setImage(DBeaverIcons
				.getImage((style & SWT.HORIZONTAL) == SWT.HORIZONTAL ? UIIcon.SEPARATOR_H : UIIcon.SEPARATOR_V));
	}
	...
	public static void packColumns(@NotNull Tree tree, boolean fit, @Nullable float[] ratios) {
		...
		try {
			...
			// TODO: it looks like SWT error. Sometimes tree items are disposed and NPE is
			// thrown from column.pack
...
//                }
	...
	public static void showMessageBox(final Shell shell, final String title, final String info, final int messageType) {
		...
		if (icon != null) {
			...
			messageBoxRunnable = () -> MessageBoxBuilder.builder(shell != null ? shell : getActiveWorkbenchShell())
					.setTitle(title).setMessage(info).setReplies(Reply.OK).setDefaultReply(Reply.OK)
					.setPrimaryImage(finalIcon).showMessageBox();
		} else {
	...
	public static boolean confirmAction(@Nullable Shell shell, String title, String message, @NotNull DBPImage image) {
		...
		syncExec(() -> reply[0] = MessageBoxBuilder.builder(shell != null ? shell : getActiveWorkbenchShell())
				.setTitle(title).setMessage(message).setReplies(Reply.YES, Reply.NO).setDefaultReply(Reply.NO)
				.setPrimaryImage(image).showMessageBox());
	...
	public static Label createControlLabel(Composite parent, String label, int hSpan) {
		...
		// Vert align center. Because height of single line control may differ from
		// label height. This makes form ugly.
	...
	@NotNull
	public static Text createLabelText(@NotNull Composite parent, @NotNull String label, @Nullable String value,
			int style, @Nullable Object layoutData) {
	...
	@NotNull
	public static Spinner createLabelSpinner(@NotNull Composite parent, @NotNull String label, int value, int minimum,
			int maximum) {
	...
	public static ToolItem createToolItem(ToolBar parent, String text, DBPImage icon,
			SelectionListener selectionListener) {
		return createToolItem(parent, text, icon != null ? DBeaverIcons.getImage(icon) : null, selectionListener);
	}

	public static ToolItem createToolItem(ToolBar parent, String title, String text, DBPImage icon,
			SelectionListener selectionListener) {
		ToolItem toolItem = createToolItem(parent, text, icon != null ? DBeaverIcons.getImage(icon) : null,
				selectionListener);
		if (title != null) {
	...
	public static ToolItem createToolItem(ToolBar parent, String text, Image icon,
			SelectionListener selectionListener) {
...
//    }
	...
	@NotNull
	public static Button createDialogButton(@NotNull Composite parent, @Nullable String label,
			@Nullable SelectionListener selectionListener) {
		return createDialogButton(parent, label, null, null, selectionListener);
	}

	@NotNull
	public static Button createDialogButton(@NotNull Composite parent, @Nullable String label, @Nullable DBPImage icon,
			@Nullable String toolTip, @Nullable SelectionListener selectionListener) {
		Button button = new Button(parent, SWT.PUSH);
		button.setText(label);
		...
		try {
			...
			widthHint = org.eclipse.jface.dialogs.Dialog.convertHorizontalDLUsToPixels(gc.getFontMetrics(),
	...
	@NotNull
	public static Button createRadioButton(@NotNull Composite parent, @Nullable String label, @NotNull Object data,
			@Nullable SelectionListener selectionListener) {
	...
	public static void showPreferencesFor(Shell shell, Object element, String... defPageID) {
		...
		if (element == null) {
			propDialog = PreferencesUtil.createPreferenceDialogOn(shell, defPageID[0], defPageID, null,
					PreferencesUtil.OPTION_NONE);
		} else {
			propDialog = PreferencesUtil.createPropertyDialogOn(shell, element, defPageID[0], null, null,
					PreferencesUtil.OPTION_NONE);
		}
		if (propDialog != null) {
			propDialog.open();
	...
	public static Combo createDelimiterCombo(Composite group, String label, String[] options, String defDelimiter,
			boolean multiDelims) {
	...
	public static void run(IRunnableContext runnableContext, boolean fork, boolean cancelable,
			final DBRRunnableWithProgress runnableWithProgress) throws InvocationTargetException, InterruptedException {
	...
	public static void runInUI(IRunnableContext context, final DBRRunnableWithProgress runnable) {
		try {
			PlatformUI.getWorkbench().getProgressService().runInUI(context,
					monitor -> runnable.run(RuntimeUtils.makeMonitor(monitor)),
	...
	public static Color getConnectionColorByRGB(String rgbStringOrId) {
		...
		if (Character.isAlphabetic(rgbStringOrId.charAt(0))) {
			...
			if (connectionColor.getBlue() == 255 && connectionColor.getRed() == 255
	...
	public static void fixReadonlyTextBackground(Text textField) {
		...
		// https://bugs.eclipse.org/bugs/show_bug.cgi?id=340889
		if (false) {
	...
	/**
	 * Returns whether the given color is dark or light depending on the colors
	 * grey-scale level.
	 */
	...
	public static void expandAll(AbstractTreeViewer treeViewer) {
		...
		try {
			// Do not use expandAll(true) as it is not supported by Eclipse versions before
			// 2019
