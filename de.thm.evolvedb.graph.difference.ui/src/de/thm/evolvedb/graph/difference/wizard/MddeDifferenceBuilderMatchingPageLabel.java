...
public class MddeDifferenceBuilderMatchingPageLabel extends WizardPage {
	...
	protected MddeDifferenceBuilderMatchingPageLabel(String pageName, MddeDifferenceBuilderController controller) {
		super(pageName);
	...
	public void createControl(Composite parent) {
		...
		listener = new ModifyListener() {
			...
			public void modifyText(ModifyEvent arg0) {
				...
				List<Correspondence> tableCorrespondence = elist.stream().filter(e -> e.getMatchedA() instanceof EdgeLabel)
				...
				for (Correspondence correspondence : tableCorrespondence) {
					...
					if (objectA instanceof EdgeLabel) {
						nameA = ((EdgeLabel) objectA).getName();
						nameB = ((EdgeLabel) objectB).getName();
		...
		addCorrespondence.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				List<EObject> matchingA = matching.getUnmatchedA().stream().filter(e1 -> e1 instanceof EdgeLabel)
						.collect(Collectors.toList());
				List<EObject> matchingB = matching.getUnmatchedB().stream().filter(e1 -> e1 instanceof EdgeLabel)
						.collect(Collectors.toList());
		...
		tcUnmatched.setText("Unmatched Label");
		tcUnmatched.pack();
...
//					dialog.setMessage("Do you really want to do this?");
	...
	private void addUnmatchedElements(EList<EObject> elistUnmatched) {
		...
		for (EObject e : elistUnmatched) {
			...
			if (e instanceof EdgeLabel) {
				name = ((EdgeLabel) e).getName();
